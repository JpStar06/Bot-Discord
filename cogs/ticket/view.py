...
class TicketBuilderView(discord.ui.View):
    ...
    async def salvar(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await services.editar_ticket(interaction.guild.id, self.ticket_id, self.title, self.description, self.color, self.image)

        await interaction.response.send_message(f"✏️ Embed `{self.ticket_id}` atualizado!", ephemeral=True)