...
class Mods(commands.Cog):
    ...
    adm = app_commands.Group(name="adm", description="Comandos de administração")
    ...
    async def mutar(self, interaction: discord.Interaction, user: discord.Member, minutos: int):
        ...
        await user.timeout(timedelta(minutes=minutos))
        await interaction.response.send_message(f"🔇 {user.mention} foi mutado por **{minutos} minutos**.")
    ...
    async def desmutar(self, interaction: discord.Interaction, user: discord.Member):
        ...
        await interaction.response.send_message(f"🔊 {user.mention} foi desmutado.")
